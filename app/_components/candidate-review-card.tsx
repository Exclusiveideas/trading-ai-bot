    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-1 text-xs">
          <h4 className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2">Price Levels</h4>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
            <span className="text-muted-foreground">Entry</span>
            <span className="font-mono tabular-nums">{keyPriceLevels.entry.toFixed(5)}</span>
            <span className="text-muted-foreground">Stop Loss</span>
            <span className="font-mono tabular-nums text-red-400">{keyPriceLevels.stopLoss.toFixed(5)}</span>
            <span className="text-muted-foreground">Take Profit</span>
            <span className="font-mono tabular-nums text-emerald-400">{keyPriceLevels.takeProfit.toFixed(5)}</span>
        ...
        <div className="text-xs">
          <h4 className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2">Context</h4>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
            <span className="text-muted-foreground">Trend</span>
            <span>{contextSnapshot.trendState ?? "N/A"}</span>
            <span className="text-muted-foreground">Support</span>
            <span className="font-mono tabular-nums">{contextSnapshot.nearestSupport?.toFixed(5) ?? "N/A"}</span>
            <span className="text-muted-foreground">Resistance</span>
            <span className="font-mono tabular-nums">{contextSnapshot.nearestResistance?.toFixed(5) ?? "N/A"}</span>
            <span className="text-muted-foreground">RSI</span>
            <span className="font-mono tabular-nums">{contextSnapshot.rsi?.toFixed(1) ?? "N/A"}</span>
            <span className="text-muted-foreground">ATR</span>
            <span className="font-mono tabular-nums">{contextSnapshot.atr?.toFixed(5) ?? "N/A"}</span>
          </div>
        </div>
        ...
        <div className="text-xs">
          <h4 className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2">Outcome</h4>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
            ...
            <Badge
              ...
              className={cn(
                ...
                outcome.outcome === "win" && "border-emerald-500/50 text-emerald-400",
                outcome.outcome === "loss" && "border-red-500/50 text-red-400",
                outcome.outcome === "pending" && "border-amber-500/50 text-amber-400",
                outcome.outcome === "breakeven" && "border-zinc-500/50 text-zinc-400",
              )}
            >
              {outcome.outcome.toUpperCase()}
            </Badge>
            <span className="text-muted-foreground">R-Multiple</span>
            <span className="font-mono tabular-nums">{outcome.rMultiple?.toFixed(2) ?? "N/A"}</span>
